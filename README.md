...
A Zone Program can be anything ranging from a static loop to a responsive program. A few examples are programs that scroll a dot in a loop, or output a rainbow loop, or output a notification using a light effect. Zone programs have the concept of Inputs, which are parameters that can be changed by the user of a program to manipulate things inside a program. A program is responsible for driving the lights in a zone. The zones and lighting controllers are static objects. A zone program runs on a zone and uses a lighting controller to output the lights in a zone. Examples are included with the project for a Rainbow program, a ScrollDot program, and a StaticColor program.

Zone programs are loaded from external assemblies. To create a new zone program, create a new C# class library project, and add a reference to the ZoneLighting assembly. This assembly gives access to the LoopingZoneProgram and ReactiveZoneProgram classes. If the program needs to loop something over and over, create a subclass of LoopingZoneProgram. If the program is simply doing things based on external stimuli, subclass ReactiveZoneProgram. Add some inputs using the AddInput or AddMappedInput method. An input needs a delegate that will determine what happens with the input value. A mapped input simply assigns the input value to a variable. See the example programs Rainbow, ScrollDot, and StaticColor for more details.
